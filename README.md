...
| [`examples/durable-account`](examples/durable-account) | The typed account served durably from a single `durable_object!` declaration. |
...
`Handles<C>` is a *pure* decider. When a command needs to consult something to
decide — a clock, a pricing client, a policy object — implement
`HandlesIn<C, Env>` instead. Build the service with `Service::with_env(store, env)`
and dispatch with `Service::dispatch`; the service hands your async handler a
shared reference to the environment. The append still goes through the same
optimistic-concurrency path, so the environment is for *reads that inform the
decision*, not for hidden writes.

```rust
struct Policy { daily_limit: u64 }

#[derive(CommandKind)] struct Withdraw(u64);

#[streamlet::async_trait]
impl HandlesIn<Withdraw, Policy> for Vault {
    async fn handle(&self, Withdraw(amount): Withdraw, policy: &Policy)
        -> Result<Vec<VaultEvent>, VaultError>
    {
        if amount > policy.daily_limit {
            return Err(VaultError::ExceedsLimit { limit: policy.daily_limit });
        }
        Ok(vec![VaultEvent::Withdrawn { amount }])
    }
}

let vault = Service::<Vault, _, Policy>::with_env(MemoryStore::new(), Policy { daily_limit: 100 });
vault.dispatch("v1", Withdraw(50)).await?;   // ok
vault.dispatch("v1", Withdraw(200)).await?;  // ServiceError::Rejected(ExceedsLimit { .. })
```

Pure (`Handles<C>` → `submit`) and dependency-driven (`HandlesIn<C, Env>` →
`dispatch`) commands coexist on the same aggregate.

### One declaration, durable too

`declare_service!` wires the in-process path. Its durable sibling,
`streamlet_restate::durable_object!`, takes the *same* aggregate + command list
and emits a full Restate Virtual Object — the `#[restate_sdk::object]` trait and
its server impl — so you don't hand-write any handler boilerplate. Each method
submits the command through `ctx.run`, so Restate journals the append and
replays it on retry, and the rejection / infrastructure split survives all the
way through the durable executor.
...
// Generates `trait AccountObject` + `struct AccountObjectServer`:
...
Command types just need `#[derive(CommandKind)]`, `Clone`, and `serde`. See
[`examples/durable-account`](examples/durable-account).
...
# Durable typed account (generated by durable_object!)
